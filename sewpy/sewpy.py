...
- run() now returns a dict containing several objects, such as the output astropy table,
    catfilepath, workdir, and logfilepath.
...
- appropriate warnings and behaviour when a workdir already exists,
    or when you rerun on the same file
...
class SEW:
    ...
    def __init__(self, workdir=None, sexpath="sex", params=None, config=None, configfilepath=None,
        ...
        :param sexpath: path to the sextractor executable (e.g., "sex" or "sextractor", if in your
                        PATH)
        :param params: the parameters you want SExtractor to measure (i.e., what you would write in
        ...
        :type config: dict
        :param configfilepath: specify this if you want me to use an existing SExtractor config file
                               as "default" (instead of the sextractor -d one)
        :param nice: niceness with which I should run SExtractor. Use e.g. ``19`` for set lowest
                     priority.
        :type nice: int

        :param loglevel: verbosity, e.g. the python-level logging threshold for the sewpy module
                         logger.
                         For example, set this to "WARNING" and sewpy will no longer log simple
                         INFOs.Choices are "DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL".
        ...
        .. warning:: When using *vector*-type params resulting in multiple columns (such as
                     "FLUX_RADIUS(3)"
                     in the example above), do not put these in the last position of the params
                     list, otherwise astropy fails reading the catalog! This is probably due to the
                     fact that the SExtractor header doesn't give a hint that multiple columns are
                     expected when a vector-type param comes last. A workaround would be way too
                     complicated.
        ...
        if workdir is not None:
            ...
            if os.path.isdir(workdir):
                # logger.warning("SExtractor workdir '%s' exists, be careful! I will
                # (maybe silently) delete or overwrite stuff." % (workdir))
                pass
            else:
        ...
        # self._clean_workdir()
        # No, don't clean it ! This is an obvious race conditions when several
        # processes use the same workdir !
    ...
    def get_version(self):
        ...
        try:
            ...
            raise RuntimeError("Could not run SExtractor. Is the path '%s' correct ? "
                               "If not, specify sexpath='/path/to/sextractor'" % self.sexpath)
    ...
    def _set_instance_config(self):
        ...
        Called by __init__(). If needed, you could still mess with this config after __init__()
        has run.
        """
    ...
    def _get_image_filepath(self, imgname):
        """
        Changes from image to image
        """
    ...
    def _write_params(self, force=False):
        ...
        if force or not os.path.exists(self._get_params_filepath()):
            ...
            self._created_files.append(self._get_params_filepath())
        else:
    ...
    def _write_default_config(self, force=False):
        ...
        if force or not os.path.exists(self._get_config_filepath()):
            p = subprocess.Popen([self.sexpath, "-d"], stdout=subprocess.PIPE,
            ...
            self._created_files.append(self._get_config_filepath())
        else:
...
1 2 1""")
            ...
            self._created_files.append(self._get_conv_filepath())
        else:
            logger.debug("Default conv file already exists, I don't overwrite it.")

    def _write_image_data(self, data, imgname):
    ...
    def __call__(self, imgfilepath, imgname=None, assoc_cat=None, assoc_xname="x", assoc_yname="y",
        ...
        :param assoc_cat:  optional input catalog (astropy table), if you want to use the
                           ASSOC helper
        :param assoc_xname: x coordinate name I should use in the ASSOC helper
        :param assoc_yname: idem

        :param returncat: by default I read the SExtractor output catalog and return it as an
                          astropy table.
        ...
        :type prefix: string
        :param writelog: if True I save the sextractor command line input and output into a
                         dedicated log file in the workdir.
        ...
        Everything related to this particular image stays within this method, the SExtractor
        instance (in particular config) is not modified !
        """
        ...
        if os.path.exists(self._get_cat_filepath(imgname)):
            logger.warning("Output catalog %s already exists, I will overwrite it" % (
        ...
        if assoc_cat is not None:
            ...
            if "ASSOCSELEC_TYPE" in imgconfig:
                raise RuntimeError(
                    "Sorry, you cannot mess with ASSOCSELEC_TYPE yourself when using the helper. "
                    "I'm using MATCHED.")
        ...
        if writelog:
            ...
            logfile.write("\n")
            logfile.close()
            self._created_files.append(self._get_log_filepath(imgname))
        ...
        if "All done" not in err:
            logger.warning(
                "Ouch, something seems wrong, check SExtractor log: %s" % self._get_log_filepath(
                    imgname))
        ...
        if returncat:
            if assoc_cat is None:
                ...
                if len(duplassoc) > 0:
                    logger.warning(
                        "%i sources from the SExtractor catalog are strange duplicates (bug ?), "
                        "I discard them." % len(duplassoc))
                ...
                if joined.masked:
                    ...
                    joined[prefix + "assoc_flag"] = joined[joined.colnames[-1]].mask is False
    ...
    def __exit__(self, type, value, traceback):
        """
        Remove created files on exit
        """
        self.clean()

    def clean(self):
        """
        Remove the files that have been created if we are allowed
        """
        if self._clean_files:
            while len(self._created_files) > 0:
                created_file = self._created_files.pop()
    ...
    # We give this fullparamtxt here as some earlier versions of sextractor are not able to
    # spit it out.
